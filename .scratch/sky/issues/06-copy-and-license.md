...
- **Residue found in review**: `frontend/src/locales/*.js` (11 packs) and `frontend/src/instr/*` (9 packs) still exist at HEAD as dead code — nothing imports them and `i18n-core.js` keeps its dictionary empty forever — yet they hold dozens of stale upstream strings ("Sign in with passkey", "Admin dashboard", openGym mentions). Not user-visible, so the grep criterion holds for anything rendered; the packs themselves are ticket 05 cleanup that hasn't fully landed yet.
