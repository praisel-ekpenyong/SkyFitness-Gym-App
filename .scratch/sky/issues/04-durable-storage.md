...
- **Manual boxes verified on the built app** (`vite preview` + Playwright, 2026-08-24): logged a 70 kg weigh-in through the UI → after the debounce, localStorage and IndexedDB held the same snapshot (identical `_ts`). Cleared only `gym_state_v1` from localStorage and reloaded → boot restored the weigh-in from IndexedDB and repopulated localStorage under a fresh timestamp; history visible on Home. Nag row appeared under *Your data* (no export stamp, history present); tapping it downloaded `opengym-backup-2026-08-24.json`, stamped `S.lastExport`, and removed the row. All six acceptance boxes now checked for real.
